pipeline {
    ...
    stages {
        ...
        stage ("Helm install") {
            steps {
               script{
                    sh "helm upgrade --install mychart mychart-path --set image.repository=827648740654.dkr.ecr.us-east-1.amazonaws.com/my-docker-repo,image.tag=15 --namespace helm-deployment"
